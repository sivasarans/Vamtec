class App {
    ...
    addData() {
        ...
        // Set Date Added as the current date and time
        const dateAdded = new Date().toISOString();  // ISO string format for easy storage and sorting
    
        // Set Date Expiry as 7 days from the current date
        const expiryDate = new Date();
        expiryDate.setDate(expiryDate.getDate() + 7);
        const dateExpiry = expiryDate.toISOString(); // Also stored in ISO format
    
        // Prepare data object with the new fields
        const data = { 
            username, 
            email, 
            password, 
    ...
    //                 <td></td>
    //                 <td></td>
    //                 <td class="border border-gray-300 p-2"> <!-- Separate column for delete button -->
    //                     <button onclick="app.deleteData(${item.id})" class="text-red-500 hover:text-red-700">🗑️</button>
    //                 </td>
    //             `;
    //             list.appendChild(row);
    ...
    //                     <span class="edit-icon" onclick="app.editData(${item.id}, 'city', '${item.city}')">✎</span>
    //                 </td>
    //                 <td class="border border-gray-300 p-2">${new Date(item.dateAdded).toLocaleString()}</td> <!-- Date Added column -->
    //                 <td class="border border-gray-300 p-2">${new Date(item.dateExpiry).toLocaleDateString()}</td> <!-- Date Expiry column -->
    //                 <td class="border border-gray-300 p-2"> <!-- Separate column for delete button -->
    //                     <button onclick="app.deleteData(${item.id})" class="text-red-500 hover:text-red-700">🗑️</button>
    //                 </td>
    //             `;
    ...
    displayData() {
        ...
        request.onsuccess = (e) => {
            ...
            e.target.result.forEach((item) => {
                ...
                const daysRemaining = Math.ceil((expiryDate - currentDate) / (1000 * 60 * 60 * 24)); // Difference in days
    
                // Apply row color based on days remaining
                if (daysRemaining >= 6) {
                ...
                row.innerHTML = `
                    ...
                    <td class="border border-gray-300 p-2">${new Date(item.dateAdded).toLocaleString()}</td> <!-- Date Added column -->
                    <td class="border border-gray-300 p-2">${expiryDate.toLocaleDateString()}</td> <!-- Date Expiry column -->
                    <td class="border border-gray-300 p-2"> <!-- Separate column for delete button -->
                        <button onclick="app.deleteData(${item.id})" class="text-red-500 hover:text-red-700">🗑️</button>
                    </td>
                `;
