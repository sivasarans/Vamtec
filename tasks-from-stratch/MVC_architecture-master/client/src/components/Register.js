...
import { fetchRecords, addOrUpdateRecord, deleteRecord } from '../redux/registerSlice';
...
const Register = () => {
  ...
  const dispatch = useDispatch();
    ...
    const { registerData, loading, error } = useSelector((state) => state.register);
    const handleDelete = (id) => {dispatch(deleteRecord(id)).then(({ payload }) => alert(payload));};
...
const handleSubmit = async (e) => {
  e.preventDefault(); console.log("Form Data ( before check ):", formData); // Debugging form data
  if (validateForm()) {
    console.log("Form Data ( after valid ):", formData); // Debugging form data
    dispatch(addOrUpdateRecord({ formData, editId, file })).then(({ payload }) => alert(payload));
    setFormData({
      name: '', organization: '', email: '', password: '', num_users: '',
      mobile: '', country_name: '', state_name: '', expiry_date: ''
    });
