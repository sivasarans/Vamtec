...
import { fetchStatesThunk, addStateThunk, updateStateThunk, deleteStateThunk } from "../redux/stateSlice";
import { fetchCountries } from "../services/stateService";
...
function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      if (editId) {
        dispatch(updateStateThunk({ id: editId, stateData }));
        alert("State updated successfully!");
      } else {
        dispatch(addStateThunk(stateData));
          ...
          <tbody>
            {states.map((state) => (
              <tr key={state.id}>
                ...
                <td>
                  <button onClick={() => handleEdit(state)} className="text-blue-500">
                    Edit
                  </button>
                </td>
                <td>
