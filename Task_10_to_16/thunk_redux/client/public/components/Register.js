...
function Register() {
    ...
    const [file, setFile] = useState(null);
        ...
        const fetchRecords = async () => {
          try {
            const response = await axios.get('http://localhost:5000/countries/register'); // Update with your actual data URL
            console.log("object(register data):",response)
  ...
  if (validateForm()) {
    ...
    useEffect(() => {
      ...
      const fetchStates = async (countryName) => {
        ...
        try {
          const response = await axios.get(`http://localhost:5000/countries/fetchStateByCountry/${countryName}`);
          const data = await response.data;
